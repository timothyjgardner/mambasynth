...
------------
    ...
    Target encoder  : Input → Linear(D_in→D) → Mamba (N layers) → LayerNorm → D-dim reps
                      (exponential moving average of context encoder, no gradients)
    Predictor       : context_rep[t] → small Mamba (M layers) → LayerNorm → Linear → D-dim
    Loss            : MSE(predictor(context_rep)[:-1], target_rep[1:])
                      i.e. predict the target's representation one step ahead

The target encoder is updated via EMA after each optimiser step, with a cosine
momentum schedule from τ_base (0.996) to 1.0.

Usage
-----
    python mamba_jepa.py                                   # train with defaults
    python mamba_jepa.py --epochs 500 --n-layers 7         # override defaults
...
class CausalMambaJEPA(nn.Module):
    ...
    n_layers : int
        Number of Mamba layers in each encoder.
    d_state : int
    ...
    def forward(self, x):
        ...
        Returns
        -------
        pred_reps   : (batch, seq_len-1, d_model) — predictor output (shifted)
        target_reps : (batch, seq_len-1, d_model) — target encoder output (shifted)
        """
        ...
        # Next-step alignment: pred[t] should match target[t+1]
...
def evaluate(model, loader, device, amp_dtype):
    """Returns (loss, cosine_similarity, target_std)."""
    model.eval()
    total_loss = 0.0
    total_cos = 0.0
    total_tgt_std = 0.0
...
def main():
    ...
    pred_label = f"MLP" if pred_type == 'mlp' else f"{args.predictor_n_layers}L Mamba"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_best:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'cos_sim': cos_sim,
                'args': {**vars(args), 'model_type': 'mamba_jepa',
                         'feature_dim': full_ds.feature_dim},
            }, args.checkpoint)
